...
class AutoNumberManager(models.Manager):
    def recalculate_max_number(self, document_class):
        doctype = document_class.document_type_name()

        try:
            doc = document_class.objects.filter(doctype=doctype).order_by(
                '-issue_date', '-number')[0]
        except IndexError:
            self.model.objects.filter(document_type=doctype).delete()
        else:
            try:
                an = self.model.objects.get(document_type=doctype)
