# Django settings for sellnov project.
import os

from configvars import config, secret

from sellnov import plugins
...
STATIC_ROOT = config("STATIC_ROOT", os.path.join(PROJECT_DIR, "static"))
...
EXTRA_TEMPLATES_DIR = config("EXTRA_TEMPLATES_DIR", "")
TEMPLATES_DIRS = [
    os.path.join(PROJECT_DIR, "templates"),
]
if EXTRA_TEMPLATES_DIR:
    TEMPLATES_DIRS.insert(0, EXTRA_TEMPLATES_DIR)
...
SELLNOV_APPS = list(filter(None, config("SELLNOV_APPS", "").split(",")))
...
INSTALLED_APPS = tuple(
    BASE_APPS + SELLNOV_BASE_APPS + SELLNOV_APPS + plugins.discover_apps()
)
...
SELLNOV_BUSINESS_ENTITY_MODEL = "userprofile.BusinessEntity"
