...
class Document(models.Model):
    ...
    owner = models.ForeignKey('userprofile.BusinessEntity', verbose_name='Podmiot')
    ...
    def as_pdf(self, extra_context=None):
        import sellnov.pdf
        return sellnov.pdf.create_pdf(self, '%s/%s.rml' % (self._meta.app_label,
            self.doctype), extra_context=extra_context)
    ...
    def total_gross(self):
        try:
            return self.line_set.aggregate(Sum('total_gross'))['total_gross__sum']
        except KeyError:
            return 0
